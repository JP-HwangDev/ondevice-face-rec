...
class CameraManager: NSObject, ObservableObject {
    ...
    private let frameSkipInterval = 2 // Process every 2nd frame for performance
...
class FaceRecognitionViewModel: ObservableObject {
    ...
    // Liveness & Emotion tracking
    @Published var smileConfidence: Float = 0.0
    @Published var isBlinking: Bool = false
    @Published var isLivenessVerified: Bool = false
    ...
    private var lastUnknownEmbedding: [Float]?
    ...
    private func setupModel() {
        do {
            ...
            self.model = try Facenet6(configuration: config)
        } catch {
    ...
    private func updateFaceResults(with observations: [VNFaceObservation]) {
        ...
        for (index, obs) in observations.enumerated() {
            ...
            if let landmarks = obs.landmarks {
                ...
                // Estimate smile confidence from mouth landmarks
                if let outerLips = landmarks.outerLips?.normalizedPoints, outerLips.count >= 2 {
                    ...
                    let mouthWidth = sqrt(pow(left.x - right.x, 2) + pow(left.y - right.y, 2))
                    self.smileConfidence = Float(mouthWidth * 5.0) // Heuristic
                }
                
                // Estimate eye closure from landmarks (Blink Detection)
                if let leftEye = landmarks.leftEye?.normalizedPoints, leftEye.count >= 4,
                   let rightEye = landmarks.rightEye?.normalizedPoints, rightEye.count >= 4 {
                    
                    func getEyeRatio(_ pts: [CGPoint]) -> Float {
                        // Very simplified ratio: height / width
                        let width = sqrt(pow(pts[0].x - pts[3].x, 2) + pow(pts[0].y - pts[3].y, 2))
                        let height = sqrt(pow(pts[1].x - pts[5].x, 2) + pow(pts[1].y - pts[5].y, 2))
                        return Float(height / width)
                    }
                    
                    let leftRatio = getEyeRatio(leftEye)
                    let rightRatio = getEyeRatio(rightEye)
                    let avgRatio = (leftRatio + rightRatio) / 2.0
                    
                    // Threshold for closed eye is typically around 0.2
                    if avgRatio < 0.18 && !self.blinkDetected {
                        self.blinkDetected = true
                        self.lastBlinkTime = Date()
                    } else if avgRatio > 0.22 && self.blinkDetected {
                        self.blinkDetected = false
                        self.isLivenessVerified = true
                    }
                }
            }
            ...
            if !embedding.isEmpty {
                ...
                await MainActor.run {
                    if index < self.detectedFaces.count, let best = topMatches.first {
                        ...
                        // Margin check: best must be at least 0.05 above second-best
                        let hasMargin: Bool
                        if topMatches.count >= 2 {
                            hasMargin = (best.similarity - topMatches[1].similarity) >= 0.05
                        } else {
                            hasMargin = true
                        }
                        ...
                        // Update recognized user for auto-attendance (employees only)
                        if effectiveSimilarity > 0.85 && hasMargin {
                            let isVisitor = self.store.users.first(where: { $0.name == best.name })?.userType == .visitor
                            if !isVisitor {
                                self.updateRecognitionHistory(name: best.name)
                                
                                // Smile-to-Action and Voice Feedback (only for eligible attendance)
                                let attendanceStatus = self.store.getTodayStatus(for: best.name)
                                if attendanceStatus != .checkedOut,
                                   self.smileConfidence > 0.8 && self.isLivenessVerified && !self.shouldAutoAttendance {
                                    self.triggerAttendance(for: best.name)
                                }
                            }
                        }
                    }
    ...
    private func triggerAttendance(for name: String) {
        self.shouldAutoAttendance = true
        self.recognizedUserName = name
        
        // Voice Feedback
        let utterance = AVSpeechUtterance(string: "こんにちは、\(name)さん。笑顔を確認しました。")
    ...
    private func applySmoothResults(at index: Int, newName: String, newSimilarity: Float, embedding: [Float]) {
        let alpha: Float = 0.5
        let matchThreshold: Float = 0.82
        let dropThreshold: Float = 0.75 // Hysteresis: must drop below this to become unknown
        var currentFace = self.detectedFaces[index]
        let smoothedSim = (newSimilarity * alpha) + (currentFace.rawSimilarity * (1.0 - alpha))
        ...
        if smoothedSim > matchThreshold || (isCurrentlyRecognized && smoothedSim > dropThreshold && currentFace.name == newName) {
            ...
            // Non-linear score mapping for better UX (Stricter mapping)
            // 0.82 -> 70% | 0.85 -> 80% | 0.9+ -> 92-100%
            let displayScore: Int
            if smoothedSim >= 0.9 {
                displayScore = Int(92 + (smoothedSim - 0.9) * 80)
            } else if smoothedSim >= 0.85 {
                displayScore = Int(80 + (smoothedSim - 0.85) * 240)
            } else {
                displayScore = Int(70 + (smoothedSim - 0.82) * 333) // (0.82, 70) to (0.85, 80)
            }
            currentFace.score = min(100, max(0, displayScore))
        } else {
            // Capture unknown embedding for visitor registration
            self.lastUnknownEmbedding = embedding
            
            // If it's a completely different person (similarity < 0.75), drop score fast
            let penalty: Float = smoothedSim < 0.75 ? 0.4 : 0.75
    ...
    private func findTopMatchesOptimized(for embedding: [Float], count: Int = 3) async -> [Candidate] {
        ...
        // Primary: centroid-based matching (fast & robust)
        for (name, centroid) in userCentroidCache {
            let centroidSim = cosineSimilarity(embedding, centroid)
            
            // Secondary: also check top-k individual embeddings for edge cases
            var bestIndividualSim: Float = -1.0
            if let sigs = userEmbeddingsCache.first(where: { $0.name == name })?.embeddings {
                // Sample up to 20 signatures for speed
            ...
            // Blend: 60% centroid, 40% best individual
            let blendedSim = centroidSim * 0.6 + bestIndividualSim * 0.4
    ...
    private func extractEmbeddingOptimized(from buffer: CVPixelBuffer, rect: CGRect, landmarks: VNFaceLandmarks2D? = nil) async -> [Float] {
        ...
        let scale = 160.0 / max(croppedImage.extent.width, croppedImage.extent.height)
        let finalImage = croppedImage.transformed(by: CGAffineTransform(scaleX: scale, y: scale))

        guard let inputValues = bufferToFloatsOptimized(from: finalImage) else { return [] }

        do {
            let mlArray = try MLMultiArray(shape: [1, 160, 160, 3], dataType: .float32)
            let ptr = mlArray.dataPointer.assumingMemoryBound(to: Float.self)
            inputValues.withUnsafeBufferPointer { buffer in
                ptr.update(from: buffer.baseAddress!, count: buffer.count)
            }

            let output = try await model.prediction(input: Facenet6Input(input: mlArray))
            guard let first = output.featureNames.first,
                  let embeddings = output.featureValue(for: first)?.multiArrayValue else { return [] }
    ...
    private func bufferToFloatsOptimized(from image: CIImage) -> [Float]? {
        ...
        CVPixelBufferCreate(kCFAllocatorDefault, 160, 160, kCVPixelFormatType_32BGRA, attrs, &pixelBuffer)
        ...
        var floats = [Float](repeating: 0, count: 160 * 160 * 3)
        let pixelCount = 160 * 160

        for i in 0..<pixelCount {
            let idx = i * 4
            floats[i * 3] = (Float(data[idx + 2]) - 127.5) / 128.0      // R
            floats[i * 3 + 1] = (Float(data[idx + 1]) - 127.5) / 128.0  // G
            floats[i * 3 + 2] = (Float(data[idx]) - 127.5) / 128.0      // B
        }
        return floats
    }
    ...
    func registerVisitor(name: String, purpose: String) {
        // Collect current embedding if available from the last match attempt
        guard let embedding = lastUnknownEmbedding else { return }
        
        let newVisitor = FaceUser(
            id: UUID().uuidString,
            name: name,
            department: "Visitor",
            faceSignatures: [embedding],
            registeredAt: Date(),
            userType: .visitor,
            visitPurpose: purpose
        )
        
        store.saveUser(newVisitor)
        
        // Report to HR
        Task {
            try? await APIService.shared.reportVisitor(name: name, purpose: purpose)
            await MainActor.run {
                self.authStatus = "방문객 \(name)님 등록 완료"
            }
            // Rebuild cache to recognize them next time
            await rebuildEmbeddingsCache()
        }
    }
